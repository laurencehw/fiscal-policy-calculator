...
|------|----------------|-------------|
| **🟢 Core — validated** | Revenue scoring (static + behavioral), distributional analysis (return-level CPS microsim), dynamic scoring (FRB/US-calibrated) | Benchmarked against published CBO/JCT/Treasury scores. **Honest accuracy is published, not just the flattering cases:** fitted calibrated reference models reproduce official decompositions (2.7% revenue over 34 benchmarks, ≤3pp distributional mean, run `scripts/cold_holdout.py`), while 12 *unfitted* module reconstructions miss by 394% mean and are reported as such; genuine *out-of-sample* predictions across 22 pre-registered cases run 42.9% mean error, 6/22 within 15%, 12/22 within 25% (`python scripts/cold_holdout.py`). |
...
The calibrated tier also carries **12 module reconstructions nothing was fitted to** — the international, trade, pharma, IRS-enforcement and climate presets, wired into the scorecard in Phase E. Those miss by **394.1% mean / 57.1% median**, and the misses are kept: the universal insulin cap is off by 2,869% because of a federal-incidence bug in `pharma.py`, and the unfitted tariff cases score gross customs revenue against targets that are net of retaliation and GDP feedback. Each carries a `known_limitations` note; none was retuned.

**Where the targets come from.** The provenance pass opened the primary documents rather than inspecting the citations. Of the 46 calibrated-tier targets, **24 have now been read out of a table** — and **15 of those 24 disagree with the figure this repository carries**, one of them (the universal insulin cap) in sign: CBO scores a private-market \$35 insulin cap as *adding* about \$11.4B to the deficit, not saving \$15B. A further **15 could not be traced at all**, including both Social Security payroll targets, for which OCACT publishes only percent-of-payroll and no dollar figure in any year. No target was moved — every one has a module constant fitted to it — but the disagreement now rides on the scorecard entry (`official_10yr_billions_line_item`) and is tabulated in [`docs/VALIDATION.md`](docs/VALIDATION.md). The remaining **7 are illustrations with no official score at all** and are excluded from every count: the honest published-benchmark count across both tiers is **61**, not 68. See [`docs/VALIDATION_NOTES.md`](docs/VALIDATION_NOTES.md) §8.
