...
|------|----------------|-------------|
| **🟢 Core — validated** | Revenue scoring (static + behavioral), distributional analysis (return-level CPS microsim), dynamic scoring (FRB/US-calibrated) | Benchmarked against published scores from CBO, JCT, Treasury and SSA, plus TPC, PWBM, the Tax Foundation and CRFB where no agency scored the policy. **Honest accuracy is published, not just the flattering cases:** fitted calibrated reference models reproduce official decompositions (2.2% revenue over 30 benchmarks, or 4.2% over 31 with the one revised-target row held in place; the 7 published distributional tables span 0.0-5.9pp, two of them circular, run `python scripts/cold_holdout.py`), while 24 *unfitted* module reconstructions miss by 72% mean and are reported as such; genuine *out-of-sample* predictions across 25 pre-registered cases run 31.3% mean error, 13/25 within 15%, 18/25 within 25% (`python scripts/cold_holdout.py`). |
...
The specialized modules (TCJA, Corporate, Estate, Credits, AMT, Payroll, PTC, Capital Gains, Tax Expenditures) are parameterized so their components **reproduce a published decomposition** — usually CBO's, JCT's or Treasury's, but the fitted benchmarks also include the Social Security Trustees, PWBM and the Tax Foundation where no agency scored the policy. Across the **30** benchmarks a module is actually fitted to, the mean absolute error is **2.2%**, 30 of 30 within 15% — but that low error is **expected by construction**. These are useful as auditable, source-linked reconstructions of official scores, *not* as evidence the model would have predicted them cold.

**Never quote the 30 without saying which rows left.** Two mechanisms move rows out of this tier and both are live. First, `ScorecardSummary.revised_target_entries` is **2**: two calibrated targets were corrected through the Tier-2 revision ledger ([`fiscal_model/validation/target_revisions.py`](fiscal_model/validation/target_revisions.py)), and a constant fitted to a superseded figure is not fitted to its replacement — so the revised TCJA-AMT-relief row moves out of the fitted tier and into the reconstructions below. Held in place instead, the fitted tier reads **31 benchmarks at 4.2%, 30 of 31 within 15%**, the extra miss being that row at 66.8%. Second, **Wave 2 moved the three capital-gains scenarios out (33 → 30)**: deleting `fiscal_model/validation/scenarios.py`'s per-case behavioural tuples removed the only constants ever fitted to those three targets, so `calibrated_to_target` is now simply `False` for them and they report as reconstructions. The fitted mean *fell* 2.8% → 2.2% and the worst fitted row is now TCJA-without-the-SALT-cap at 13.9%, because the rows that left were the ones this tier had been carrying. That is a change of **composition, not of accuracy** — read it next to the reconstruction tier below or not at all.

The calibrated tier also carries **24 module reconstructions nothing was fitted to** — the international, trade, pharma, IRS-enforcement and climate presets wired in during Phase E, the eight P.L. 119-21 JCT line items added in Phase D, the one row the revision ledger moved out of the fitted tier, and the three capital-gains scenarios Wave 2 unfitted. Those miss by **72.1% mean / 40.0% median** (12 sectoral presets at 104.8%, 8 P.L. 119-21 line items at 35.8%, 3 capital-gains scenarios at 39.6%, TCJA AMT relief at 66.8%), and the misses are kept: international reference pricing is off by 646% against a target that is a RAND price statistic rather than a budget score, and the unfitted tariff cases score gross customs revenue against targets that are net of retaliation and GDP feedback. Each carries a `known_limitations` note; none was retuned. The universal insulin cap scores **+$7.0B** against CBO's own **+$11.4B** (publication 57957) — a deficit *increase* on both sides, **39.0%** below it — where the repository used to carry a -$15B saving that pointed the other way and made the row read 146%. The P.L. 119-21 block is the sharpest of these: the TCJA module reproduces CBO's \$4.6T aggregate to **0.4%** and JCT's own provision rows to **35.8%**, because its single calibration factor is fitted to the aggregate and to no individual row.

**Where the targets come from.** The provenance pass opened the primary documents rather than inspecting the citations. Of the 54 calibrated-tier targets, **32 are labelled from a primary-document row and 28 have actually been read out of one** (the other 4 cite a document nobody has re-opened) — and **13 of the 28 disagree with the figure this repository carries**. A further **15 could not be traced at all**, including both Social Security payroll targets, for which OCACT publishes only percent-of-payroll and no dollar figure in any year. **Two targets have since been moved** — not edited, but superseded through the Tier-2 revision ledger, which keeps the old figure on the record as a `superseded_by` row: the universal insulin cap from -\$15B to CBO 57957's **+\$11.4B** (a \$35 cost-sharing cap *adds* to the deficit), and TCJA AMT relief from \$450B to CRS R48286's **\$1,357.1B** (the \$450B was a five-year cost sitting in a ten-year column). Both are confirmations now. Where a target was *not* moved the disagreement still rides on the scorecard entry (`official_10yr_billions_line_item`) and is tabulated in [`docs/VALIDATION.md`](docs/VALIDATION.md). The remaining **7 are illustrations with no official score at all** and are excluded from every count: the honest published-benchmark count across both tiers is **72**, not 79. See [`docs/VALIDATION_NOTES.md`](docs/VALIDATION_NOTES.md) §7 and §8.
...
| ARP 2021, quintiles | CBO 56952 | microsim | good | 4.76 |
