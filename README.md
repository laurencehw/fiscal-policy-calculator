...
|------|----------------|-------------|
| **🟢 Core — validated** | Revenue scoring (static + behavioral), distributional analysis (return-level CPS microsim), dynamic scoring (FRB/US-calibrated) | Benchmarked against published CBO/JCT/Treasury scores. **Honest accuracy is published, not just the flattering cases:** fitted calibrated reference models reproduce official decompositions (2.7% revenue over 34 benchmarks, ≤3pp distributional mean, run `scripts/cold_holdout.py`), while 12 *unfitted* module reconstructions miss by 394% mean and are reported as such; genuine *out-of-sample* predictions across 23 pre-registered cases run 43.4% mean error, 6/23 within 15%, 12/23 within 25% (`python scripts/cold_holdout.py`). |
