...
The tail's four largest rows are now **corporate margins at 62%**, the **AGI surtax at 45%**, and those two Green Book capital-gains rows at 43% and 31%. Corporate rose from 47% in Wave 5, pre-registered: the derived path prices a percentage point of corporate rate on IRS SOI Table 11's published statutory base rather than a fitted aggregate that turns out to be a stale TY2018 vintage, and the residual that is left is a disagreement between two documents — CBO's option prices a point at \$135.7B over the window and Treasury's Green Book at \$192.8B, 42% apart, with the *larger* rate change carrying the *larger* per-point yield. Two rate cases sit between the core and the tail at 18% and 45%, both because the source states a filing-status-specific boundary the generic path cannot express; the 45% one is the AGI surtax and it did not move in Wave 5. Misses are kept, not tuned away, and each carries a documented structural cause. Three of the cases replicate laws that actually passed — the Social Security Fairness Act, the Fiscal Responsibility Act's discretionary caps and IIJA — always as *components* whose annual level or authorization schedule the CBO estimate itself states, never as a bill total no single shape can construct. Every case is **pre-registered** ([`fiscal_model/validation/preregistered.py`](fiscal_model/validation/preregistered.py)) *before* the commit that first scores it, and the tier is **CI-gated** (`--max-mean-error 20 --min-within-25pct 21`, re-derived from this battery by the workflow's own rule after Wave 5 — ceiling `ceil(15.9 × 1.25) = 20`, floor `22 - 1 = 21`, both tightening). Treat uncalibrated custom rate policies as **directional, ±15-25%**.
