...
| Biden capital income at ordinary rates | -\$289B | -\$699B | 142% | Treasury (Green Book FY2025 row) |
| Treasury 39.6% + step-up repeal | -\$322B | -\$817B | 154% | Treasury (Green Book FY2022 row) |

**25 out-of-sample cases, mean abs error 34.4%, 12/25 within 15%, 16/25 within 25%** (median 16.1%; `python scripts/cold_holdout.py`). There is deliberately no single "validated within X%" figure: the distribution has a tight core and a long tail. The model predicts ordinary and AGI-inclusive *rate* changes at conventional thresholds well (2-21%) and discretionary funding changes — now scored through a budget-authority-to-outlay spend-out model — well too (CBO Options spending rows 0-11%, the three enacted-law components 10-18%); everything behavioural — capital-gains realizations, gains at death, payroll incidence, corporate margins — it predicts badly (47-154%). Two rate cases sit between the two groups at 26% and 45%, both because the source states a filing-status-specific boundary the generic path cannot express. Misses are kept, not tuned away, and each carries a documented structural cause: capital-gains behaviour runs through a $54B flow constant and one frozen elasticity set; module revenue identities over-state a marginal base. Three of the cases replicate laws that actually passed — the Social Security Fairness Act, the Fiscal Responsibility Act's discretionary caps and IIJA — always as *components* whose annual level or authorization schedule the CBO estimate itself states, never as a bill total no single shape can construct. Every case is **pre-registered** ([`fiscal_model/validation/preregistered.py`](fiscal_model/validation/preregistered.py)) *before* the commit that first scores it, and the tier is **CI-gated** (`--max-mean-error 45 --min-within-25pct 15`). Treat uncalibrated custom rate policies as **directional, ±15-25%**, and uncalibrated capital-gains and payroll policies as not yet predictive.
...
The specialized modules (TCJA, Corporate, Estate, Credits, AMT, Payroll, PTC, Capital Gains, Tax Expenditures) are parameterized so their components **reproduce a published decomposition** — usually CBO's, JCT's or Treasury's, but the fitted benchmarks also include the Social Security Trustees, PWBM and the Tax Foundation where no agency scored the policy. Across the **33** benchmarks a module is actually fitted to, the mean absolute error is **2.8%**, 32 of 33 within 15% — but that low error is **expected by construction**. These are useful as auditable, source-linked reconstructions of official scores, *not* as evidence the model would have predicted them cold.

**Never quote the 33 without saying a 34th row left.** `ScorecardSummary.revised_target_entries` is **2**: two calibrated targets were corrected through the Tier-2 revision ledger ([`fiscal_model/validation/target_revisions.py`](fiscal_model/validation/target_revisions.py)), and a constant fitted to a superseded figure is not fitted to its replacement — so the revised TCJA-AMT-relief row moves out of the fitted tier and into the reconstructions below. Held in place instead, the fitted tier reads **34 benchmarks at 4.7%, 32 of 34 within 15%**, the extra miss being that row at 66.8%. Both readings are honest; neither is honest on its own.

The calibrated tier also carries **21 module reconstructions nothing was fitted to** — the international, trade, pharma, IRS-enforcement and climate presets wired in during Phase E, the eight P.L. 119-21 JCT line items added in Phase D, and the one row the revision ledger moved out of the fitted tier. Those miss by **76.7% mean / 41.0% median** (12 sectoral presets at 104.8%, 8 P.L. 119-21 line items at 35.8%, TCJA AMT relief at 66.8%), and the misses are kept: international reference pricing is off by 646% against a target that is a RAND price statistic rather than a budget score, and the unfitted tariff cases score gross customs revenue against targets that are net of retaliation and GDP feedback. Each carries a `known_limitations` note; none was retuned. The universal insulin cap scores **+$7.0B** against CBO's own **+$11.4B** (publication 57957) — a deficit *increase* on both sides, **39.0%** below it — where the repository used to carry a -$15B saving that pointed the other way and made the row read 146%. The P.L. 119-21 block is the sharpest of these: the TCJA module reproduces CBO's \$4.6T aggregate to **0.4%** and JCT's own provision rows to **35.8%**, because its single calibration factor is fitted to the aggregate and to no individual row.
