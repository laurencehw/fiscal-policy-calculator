...
The calibrated tier also carries **20 module reconstructions nothing was fitted to** — the international, trade, pharma, IRS-enforcement and climate presets wired in during Phase E, plus the eight P.L. 119-21 JCT line items added in Phase D. Those miss by **250.8% mean / 43.1% median**, and the misses are kept: the universal insulin cap is off by 2,869% because of a federal-incidence bug in `pharma.py`, and the unfitted tariff cases score gross customs revenue against targets that are net of retaliation and GDP feedback. Each carries a `known_limitations` note; none was retuned. The P.L. 119-21 block is the sharpest of these: the TCJA module reproduces CBO's \$4.6T aggregate to **0.4%** and JCT's own provision rows to **35.8%**, because its single calibration factor is fitted to the aggregate and to no individual row.
