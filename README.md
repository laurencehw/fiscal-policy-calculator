...
The calibrated tier also carries **20 module reconstructions nothing was fitted to** — the international, trade, pharma, IRS-enforcement and climate presets wired in during Phase E, plus the eight P.L. 119-21 JCT line items added in Phase D. Those miss by **253.2% mean / 40.7% median**, and the misses are kept: the universal insulin cap is off by 2,869% because of a federal-incidence bug in `pharma.py`, and the unfitted tariff cases score gross customs revenue against targets that are net of retaliation and GDP feedback. Each carries a `known_limitations` note; none was retuned. The P.L. 119-21 block is the sharpest of these: the TCJA module reproduces CBO's $4.6T aggregate to **0.4%** and JCT's own provision rows to **41.8%**, because its single calibration factor is fitted to the aggregate and to no individual row. Of the 54 calibrated-tier targets, **12 cite a specific table row** (`line_item`, eight of them that block); 31 are rounded headline figures, 7 are model estimates with no published score at all, and 4 are unclassified — so the honest published-target count is **47**. See [`docs/VALIDATION_NOTES.md`](docs/VALIDATION_NOTES.md) §7 and §8.
