...
|------|----------------|-------------|
| **🟢 Core — validated** | Revenue scoring (static + behavioral), distributional analysis (return-level CPS microsim), dynamic scoring (FRB/US-calibrated) | Benchmarked against published scores from CBO, JCT, Treasury and SSA, plus TPC, PWBM, the Tax Foundation and CRFB where no agency scored the policy. **Honest accuracy is published, not just the flattering cases:** fitted calibrated reference models reproduce official decompositions (2.0% revenue over 28 benchmarks, or 4.3% over 29 with the revised TCJA-AMT row held in place; the 7 published distributional tables span 0.00-7.77pp, two of them circular, run `python scripts/cold_holdout.py`), while 26 *unfitted* module reconstructions miss by 61.8% mean and are reported as such; genuine *out-of-sample* predictions across 26 pre-registered cases run 31.0% mean error, 13/26 within 15%, 19/26 within 25% (`python scripts/cold_holdout.py`). |
...
| Tax accrued gains at death | -\$536B | -\$581B | 8% | CBO Options #51 |
...
| Limit the income-tax employer-health exclusion | -\$697B | -\$530B | 24% | CBO Options #56 |
...
| Biden capital income at ordinary rates | -\$289B | -\$678B | 135% | Treasury (Green Book FY2025 row) |
| Treasury 39.6% + step-up repeal | -\$322B | -\$1,022B | 218% | Treasury (Green Book FY2022 row) |

**26 out-of-sample cases, mean abs error 31.0%, 13/26 within 15%, 19/26 within 25%** (median 15.1%; `python scripts/cold_holdout.py`). There is deliberately no single "validated within X%" figure: the distribution has a tight core and a long tail. The model predicts ordinary and AGI-inclusive *rate* changes at conventional thresholds well (2-22%) and discretionary funding changes — now scored through a budget-authority-to-outlay spend-out model — well too (CBO Options spending rows 0-11%, the three enacted-law components 10-18%). The one **tax-expenditure cap** in the tier, CBO Option 56, lands at **24%**, and its whole residual is named rather than tuned: CBO's revenue grows ~14%/yr because the dollar limit is indexed to the chained CPI-U while premiums grow faster, and the module evaluates its excess share once at the start year. **Gains at death left the tail in Wave 2**: replacing a flat $54B/yr constant with decedent wealth × an unrealized-gain share by estate size took CBO Option 51 from 84% to **8%**, the largest single move in the battery. What remains of the behavioural tail is narrower and better named — a 2pp preferential-rate change at 45%, corporate margins at 47%, payroll incidence at 54-56%, and the two step-up-elimination rows at **135%** and **218%**, where the model prices a per-decedent exclusion and *no* behavioural response to constructive realization at death, while Treasury's own score prices spousal and charitable carve-outs, the §121 residence exclusion, tangible personal property and a family-business deferral. Two rate cases sit between the two groups at 18% and 45%, both because the source states a filing-status-specific boundary the generic path cannot express. Misses are kept, not tuned away, and each carries a documented structural cause. Three of the cases replicate laws that actually passed — the Social Security Fairness Act, the Fiscal Responsibility Act's discretionary caps and IIJA — always as *components* whose annual level or authorization schedule the CBO estimate itself states, never as a bill total no single shape can construct. Every case is **pre-registered** ([`fiscal_model/validation/preregistered.py`](fiscal_model/validation/preregistered.py)) *before* the commit that first scores it, and the tier is **CI-gated** (`--max-mean-error 40 --min-within-25pct 18`, re-derived from this battery by the workflow's own rule — the floor tightened 17 → 18 when Option 56 took the within-25% count to 19). Treat uncalibrated custom rate policies as **directional, ±15-25%**, and uncalibrated capital-gains and payroll policies as not yet predictive.

One case left the battery in the provenance pass and one changed target. **Top rate to 45% was retired**: its -\$420B is in no TPC, CBO or JCT publication (TPC's full sitemap contains no 45%-ordinary-rate table at any date), so it is withdrawn rather than scored against a number nobody published — and the figure is gone from the app's score map too. **Biden capital gains was re-sourced** from an unsupported -\$456B to the FY2025 Green Book's actual combined row, -\$288.6B, with the shape corrected to that document's own definition (taxable income over \$1M; a \$5M per-donor exclusion for gains at death, not \$1M) — which moved its error from 79% to **142%**. A better target and a more faithful shape, scored honestly.

#### 2. Calibrated reference models — transparent reconstructions, not independent confirmation

The specialized modules (TCJA, Corporate, Estate, Credits, AMT, Payroll, PTC, Capital Gains, Tax Expenditures) are parameterized so their components **reproduce a published decomposition** — usually CBO's, JCT's or Treasury's, but the fitted benchmarks also include the Social Security Trustees, PWBM and the Tax Foundation where no agency scored the policy. Across the **28** benchmarks a module is actually fitted to, the mean absolute error is **2.0%**, 28 of 28 within 15% — but that low error is **expected by construction**. These are useful as auditable, source-linked reconstructions of official scores, *not* as evidence the model would have predicted them cold.

**Never quote the 28 without saying which rows left.** Three mechanisms move rows out of this tier and all are live. First, `ScorecardSummary.revised_target_entries` is **3**: three calibrated targets have been corrected through the Tier-2 revision ledger ([`fiscal_model/validation/target_revisions.py`](fiscal_model/validation/target_revisions.py)), and a constant fitted to a superseded figure is not fitted to its replacement — so the revised TCJA-AMT-relief row moves out of the fitted tier and into the reconstructions below. Held in place instead, the fitted tier reads **29 benchmarks at 4.3%, 28 of 29 within 15%**, the extra miss being that row at 66.8%. Second, **Wave 2 moved the three capital-gains scenarios out (33 → 30)**: deleting `fiscal_model/validation/scenarios.py`'s per-case behavioural tuples removed the only constants ever fitted to those three targets, so `calibrated_to_target` is now simply `False` for them and they report as reconstructions. Third, **Wave 3's L8 lane moved the two Trump tariff rows out (30 → 28)**: `universal_coverage_rate` and `china_effective_coverage` were fitted to those two benchmarks, and once the first became a Census measurement and the second was deleted outright, no `TRADE_BASELINE` constant is fitted to anything. The fitted mean *fell* 2.8% → 2.2% → **2.0%** across those three moves while nothing regressed, because every row that left was one this tier had been carrying — the two tariff rows were scoring 1.1% and 6.2% and now score 37.1% and 44.3% on an honest base. That is a change of **composition, not of accuracy** — read it next to the reconstruction tier below or not at all. The worst fitted row is still TCJA-without-the-SALT-cap at 13.9%.

The calibrated tier also carries **26 module reconstructions nothing was fitted to** — the international, trade, pharma, IRS-enforcement and climate presets wired in during Phase E (now including the two reclassified tariff rows), the eight P.L. 119-21 JCT line items added in Phase D, the one row the revision ledger moved out of the fitted tier, and the three capital-gains scenarios Wave 2 unfitted. Those miss by **61.8% mean / 38.0% median**, 5 of 26 within 15% (14 sectoral presets at 81.0%, 8 P.L. 119-21 line items at 35.8%, 3 capital-gains scenarios at 39.6%, TCJA AMT relief at 66.8%). **Read the sectoral fall as composition too**: on the constant 12-row population the sectoral subset is **87.8%**, not 81.0% — L8 took it from 104.8% to 84.6% by netting the tariff scores, and L9 then pushed it back up 3.2pp by giving FDII repeal an identity built on Treasury's own published cost. The misses are kept: international reference pricing is off by 646% against a target that is a RAND price statistic rather than a budget score, and the auto tariff is off by 82% against a −$100B target that itemises in none of the four CRFB posts it cites, while the two located primary estimates are 4-6.5× above it. Each carries a `known_limitations` note; none was retuned. The universal insulin cap scores **+$7.0B** against CBO's own **+$11.4B** (publication 57957) — a deficit *increase* on both sides, **39.0%** below it — where the repository used to carry a -$15B saving that pointed the other way and made the row read 146%. The P.L. 119-21 block is the sharpest of these: the TCJA module reproduces CBO's \$4.6T aggregate to **0.4%** and JCT's own provision rows to **35.8%**, because its single calibration factor is fitted to the aggregate and to no individual row.

**Where the targets come from.** The provenance pass opened the primary documents rather than inspecting the citations. Of the 54 calibrated-tier targets, **32 are labelled from a primary-document row and 28 have actually been read out of one** (the other 4 cite a document nobody has re-opened) — and **13 of the 28 disagree with the figure this repository carries**. A further **15 could not be traced at all**, including both Social Security payroll targets, for which OCACT publishes only percent-of-payroll and no dollar figure in any year. **Three targets have since been moved** — not edited, but superseded through the Tier-2 revision ledger, which keeps the old figure on the record as a `superseded_by` row: the universal insulin cap from -\$15B to CBO 57957's **+\$11.4B** (a \$35 cost-sharing cap *adds* to the deficit), TCJA AMT relief from \$450B to CRS R48286's **\$1,357.1B** (the \$450B was a five-year cost sitting in a ten-year column), and Pillar Two adoption from a -\$80B point to JCT JCX-22-23's **published range [-\$102.6B, +\$56.5B]**. The third is a different kind of assertion from the other two: -\$80B is the midpoint of a range the module's own header documents and appears in no JCT scenario, and picking a single scenario instead would mean picking the *rest of the world's* behaviour, which is not part of the US policy being scored. The model's -\$61.2B sits **inside** the range, distance to the nearest bound **\$0.0B**, so the 23.5% it reports against -\$80B is a distance from an editorial midpoint and not a measurement of accuracy. Where a target was *not* moved the disagreement still rides on the scorecard entry (`official_10yr_billions_line_item`) and is tabulated in [`docs/VALIDATION.md`](docs/VALIDATION.md); the estate-reform row is the first to carry an explicit **examined-and-left** verdict (`EXAMINED_NOT_REVISED`) rather than silence, because JCT's -\$429.6B totals a ten-section bill the module does not construct. The remaining **7 are illustrations with no official score at all** and are excluded from every count: the honest published-benchmark count across both tiers is **73**, not 80. See [`docs/VALIDATION_NOTES.md`](docs/VALIDATION_NOTES.md) §7 and §8.
...
| ARP 2021, quintiles | CBO 56952 | microsim | acceptable | 7.77 |
...
| P.L. 119-21, 2026-2034 avg, deciles | CBO 61367 | synthetic | good | 3.96 |

`≤ 2pp = excellent`, `≤ 5pp = good`, `≤ 10pp = acceptable`. The SALT number is **higher than before by design**: it used to read 0.00 from a table *calibrated to JCX-4-24*; it now reads 5.86 from a genuine return-level computation (the synthetic calibrated reference still matches near-exactly if forced). **The ARP number rose the same way, 4.76 → 7.77**, when Wave 3 put the Recovery Rebate on return-level data alongside the CTC and EITC: the old 4.76 was ranking one of the three components by IRS return counts and the other two by CPS tax units, and the two rankings partly cancelled. Scored consistently, the model's quintile dollar averages move from about a third of CBO's to close to them and the bundle totals \$485B, within 10% of the three provisions' actual cost — while the reported share error gets worse, because the model's bottom quintile is 38.2M tax units against CBO's ~26M households. Per the calibration-vs-prediction split above, the honest computed number is the one we report. See [`docs/VALIDATION_NOTES.md`](docs/VALIDATION_NOTES.md) for the ARP residual analysis.
