...
The specialized modules (TCJA, Corporate, Estate, Credits, AMT, Payroll, PTC, Capital Gains, Tax Expenditures) are parameterized so their components **reproduce a published decomposition** — usually CBO's, JCT's or Treasury's, but the 34 fitted benchmarks also include the Social Security Trustees, PWBM and the Tax Foundation where no agency scored the policy. Across the **34** benchmarks a module is actually fitted to, the mean absolute error is **2.7%** — but that low error is **expected by construction**. These are useful as auditable, source-linked reconstructions of official scores, *not* as evidence the model would have predicted them cold.

The calibrated tier also carries **20 module reconstructions nothing was fitted to** — the international, trade, pharma, IRS-enforcement and climate presets wired in during Phase E, plus the eight P.L. 119-21 JCT line items added in Phase D. Those miss by **82.6% mean / 43.1% median** (12 sectoral presets at 113.8%, 8 P.L. 119-21 line items at 35.8%), and the misses are kept: international reference pricing is off by 646% against a target that is a RAND price statistic rather than a budget score, and the unfitted tariff cases score gross customs revenue against targets that are net of retaliation and GDP feedback. Each carries a `known_limitations` note; none was retuned. The universal insulin cap now scores **+$7.0B** — a deficit *increase*, agreeing in sign with CBO's own +$11.4B for the same policy — against a carried target of -$15B that points the other way, so its 146% is the price of pointing the right way. The P.L. 119-21 block is the sharpest of these: the TCJA module reproduces CBO's \$4.6T aggregate to **0.4%** and JCT's own provision rows to **35.8%**, because its single calibration factor is fitted to the aggregate and to no individual row.

**Where the targets come from.** The provenance pass opened the primary documents rather than inspecting the citations. Of the 54 calibrated-tier targets, **32 are labelled from a primary-document row and 28 have actually been read out of one** (the other 4 cite a document nobody has re-opened) — and **15 of the 28 disagree with the figure this repository carries**, one of them (the universal insulin cap) in sign: CBO scores a private-market \$35 insulin cap as *adding* about \$11.4B to the deficit, not saving \$15B. A further **15 could not be traced at all**, including both Social Security payroll targets, for which OCACT publishes only percent-of-payroll and no dollar figure in any year. No target was moved — every one has a module constant fitted to it — but the disagreement now rides on the scorecard entry (`official_10yr_billions_line_item`) and is tabulated in [`docs/VALIDATION.md`](docs/VALIDATION.md). The remaining **7 are illustrations with no official score at all** and are excluded from every count: the honest published-benchmark count across both tiers is **72**, not 79. See [`docs/VALIDATION_NOTES.md`](docs/VALIDATION_NOTES.md) §7 and §8.
