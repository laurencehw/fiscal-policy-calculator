...
|------|----------------|-------------|
| **🟢 Core — validated** | Revenue scoring (static + behavioral), distributional analysis (return-level CPS microsim, on CBO's household universe where the source ranks households), dynamic scoring (FRB/US-calibrated) | Benchmarked against published scores from CBO, JCT, Treasury and SSA, plus TPC, PWBM, the Tax Foundation, CRFB, Penn Wharton and RAND where no agency scored the policy. **Honest accuracy is published, not just the flattering cases:** fitted calibrated reference models reproduce official decompositions (**1.5% over 16 benchmarks** since Wave B's provenance pass moved five more onto documents, or **11.9% over 27** with the six rows a *target revision* moved out held in place — the reading `python scripts/run_validation_dashboard.py` prints; the 7 published distributional tables span 0.00-5.86pp, two of them circular), while **39 *unfitted* module reconstructions miss by 56.7% mean** and are reported as such — **and that rise from 55.5% *is* accuracy, on the same 39 rows: it is the five tariff rows moving when retaliation left a score measured against conventional targets, registered as worse in advance. The earlier fall from 57.9% to 55.5% was composition, and on the 34 rows that tier held before Wave B the new targets read 58.3%**; genuine *out-of-sample* predictions across 26 pre-registered cases run **14.5% mean error, 11.5% median, 16/26 within 15%, 22/26 within 25%** (`python scripts/cold_holdout.py`), and that tier is itself **eight policy classes running 4.6% to 44.5%**, each now gated separately in CI. **Never quote the fitted mean without saying which rows left it**: the 21 rows that tier held before Wave B, scored on the targets Wave B leaves behind, read **9.8%**. |
...
All 26, sorted by error. Nothing is omitted: a battery that showed only its good rows would not be a validation.
...
| Treasury 39.6% + step-up repeal | -\$322B | -\$316B | 2% | Treasury (Green Book FY2022 row) |
| All ordinary rates +1pp | -\$1,185B | -\$1,207B | 2% | CBO Options #45 |
| End national community service | -\$10B | -\$11B | 3% | CBO Options #38 |
| AGI surtax 2pp (>\$100K single) | -\$1,051B | -\$1,081B | 3% | CBO Options #46 |
| AGI surtax 1pp (>\$20K single) | -\$1,440B | -\$1,333B | 7% | CBO Options #46 |
...
| Top four brackets +2pp | -\$570B | -\$654B | 15% | CBO Options #45 |
| Biden top rate 39.6% (\$400K+) | -\$246B | -\$290B | 18% | Treasury |
| IIJA 2021 (discretionary component) | +\$415B | +\$340B | 18% | CBO, S.Amdt. 2137 |
| 2pp rate cut (\$500K+) | +\$400B | +\$473B | 18% | Tax Policy Center † |
| 5pp top rate (\$1M+) | -\$700B | -\$842B | 20% | Tax Policy Center † |
| 1pp all brackets | -\$960B | -\$1,195B | 24% | JCT † |
| Warren ultra-millionaire surtax 3pp | -\$350B | -\$437B | 25% | Tax Policy Center † |
| Biden capital income at ordinary rates | -\$289B | -\$366B | 27% | Treasury (Green Book FY2025 row) |
| Medicare surcharge 2pp (>\$400K) | -\$310B | -\$409B | 32% | Treasury |
| Tax accrued gains at death | -\$536B | -\$346B | 35% | CBO Options #51 |
| Corporate rate +1pp (21%→22%) | -\$136B | -\$196B | 44% | CBO Options #64 (JCT estimate) |

† **These four targets carry no source URL**, and two of them are labelled "Illustrative estimate" in the repository's own record. `1pp all brackets` and `All ordinary rates +1pp` score the **same reform** against targets **23.5% apart** — a -\$960B rule of thumb against CBO's own printed -\$1,185B option — so the model cannot agree with both, and Wave B swapped which one it agrees with. Whether those four are revised, examined-and-left or retired is a target decision for the provenance ledger, not a modelling one.

**26 out-of-sample cases, mean abs error 14.5%, 16/26 within 15%, 22/26 within 25%** (median 11.5%; `python scripts/cold_holdout.py`). There is deliberately no single "validated within X%" figure: the distribution has a tight core and a long tail, and it is **eight policy classes** rather than one — AGI-inclusive surtax 17.6% (n=6), capital gains 18.7% (4), ordinary rate change 14.8% (4), corporate 44.5% (1), enacted-law spending 13.4% (3), discretionary spending 4.6% (5), payroll 7.8% (2), tax-expenditure cap 13.1% (1). **Each class now carries its own CI ceiling** (`--max-class-mean-error`), because a pooled mean cannot see one class regressing while the others carry it — which is what happened to the Medicare surcharge in Wave 7 and to the Warren surtax in Wave B. The model predicts ordinary and AGI-inclusive *rate* changes at conventional thresholds well (1.5-22%) and discretionary funding changes — now scored through a budget-authority-to-outlay spend-out model — well too (CBO Options spending rows 0-11%, the three enacted-law components 10-18%). The one **tax-expenditure cap** in the tier, CBO Option 56, lands at **13%** since Wave 4 gave its excess share CBO's own chained-CPI indexation instead of evaluating it once at the start year; what is left is named rather than tuned — about half of it is a **base omission** (CBO caps premiums *and* FSA/HRA/HSA contributions, and the repository's premium distribution has no account dimension) and about a fifth a **behavioural offset whose direction is now sourced and whose magnitude is not** (Wave 7 read the direction off CBO 60557 Option 56's own text; the elasticity's size remains unsourced, here and on the module's other four reforms).
