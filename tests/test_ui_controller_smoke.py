...
def test_render_data_status_surfaces_stale_bundled_seed(monkeypatch):
    monkeypatch.setattr(
        "fiscal_model.health.check_health",
        lambda: {
            "overall": "degraded",
    ...
    assert any("**FRED:** Stale bundled seed (130 days)" in text for text in st_module.markdowns)
