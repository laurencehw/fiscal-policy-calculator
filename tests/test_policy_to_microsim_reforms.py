...
def test_corporate_policy_returns_empty():
    ...
    # Corporate rate change doesn't set top-rate reform (that's individual).
    # Expected: empty dict → pilot raises UnsupportedModelPolicyError.
    assert "ctc_amount" not in reforms
    # Accept either empty (strictest) or a rate-change-only mapping with
    # no meaningful effect on individual returns.
    assert "ctc_fully_refundable" not in reforms
