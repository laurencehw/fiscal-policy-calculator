...
def _result(name: str, *, avg: float, pct: float, share: float):
    """Stand-in for ``DistributionalResult`` carrying just the fields the
    narrative helpers read."""
...
def test_build_winners_losers_partitions_groups():
    analysis = _analysis(
        _result("Bottom Quintile", avg=+50, pct=+0.10, share=+0.01),
        _result("Middle Quintile", avg=-100, pct=-0.20, share=-0.05),
        _result("Top Quintile", avg=-4500, pct=-1.20, share=-0.94),
        total=-200.0,
    )
...
def test_headline_direction_progressive_when_top_pays_more():
    analysis = _analysis(
        _result("Bottom", avg=10, pct=0.05, share=0.001),
        _result("Top", avg=20000, pct=4.5, share=0.99),
    )
    summary = build_winners_losers(analysis)
...
def test_headline_direction_regressive_when_bottom_hit_harder():
    analysis = _analysis(
        _result("Bottom", avg=200, pct=2.0, share=0.4),
        _result("Top", avg=2500, pct=0.3, share=0.6),
    )
    summary = build_winners_losers(analysis)
...
def test_headline_direction_flat_when_pct_of_income_matches():
    analysis = _analysis(
        _result("Bottom", avg=100, pct=0.50, share=0.25),
        _result("Top", avg=5000, pct=0.50, share=0.75),
    )
    summary = build_winners_losers(analysis)
...
def test_headline_sentence_phrases_top_and_bottom():
    analysis = _analysis(
        _result("Bottom Quintile", avg=+30, pct=+0.10, share=+0.01),
        _result("Top Quintile", avg=-4250, pct=-1.20, share=-0.95),
    )
...
def test_render_winners_losers_callout_with_mocked_streamlit():
    analysis = _analysis(
        _result("Bottom", avg=+30, pct=+0.10, share=+0.01),
        _result("Middle", avg=-100, pct=-0.20, share=-0.05),
        _result("Top", avg=-4500, pct=-1.20, share=-0.94),
        total=-200.0,
    )
...
def test_render_handles_no_winners_no_losers():
    ...
    analysis = _analysis(
        _result("Bottom", avg=0, pct=0, share=0),
        _result("Top", avg=0, pct=0, share=0),
    )
    st = MagicMock()
