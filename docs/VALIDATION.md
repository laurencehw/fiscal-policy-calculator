...
> **26 out-of-sample cases, mean abs error 15.2%, 16/26 within 15%, 22/26 within 25%** (median 11.4%).
...
| New 1% payroll tax (all earnings) | -$1,282B | -$1,378B | 8% | CBO Options 2025-2034 #61 (2024-12) | CBO Feb 2024 (matched) | `752f0f1`, scored `36d683f` |
| New 2% payroll tax (all earnings) | -$2,540B | -$2,745B | 8% | CBO Options 2025-2034 #61 (2024-12) | CBO Feb 2024 (matched) | `752f0f1`, scored `36d683f` |
...
| LTCG + qualified dividends +2pp | -$103B | -$93B | 10% | CBO Options 2025-2034 #47 (2024-12) | CBO Feb 2024 (matched) | `752f0f1`, scored `36d683f` |
...
| Biden capital income at ordinary rates | -$289B | -$379B | 31% | Treasury (2024-03), Green Book FY2025 table row | Green Book FY2025 | `0bcfbc3` (`.v2`, superseding `.v1`) |
| Treasury 39.6% + step-up repeal | -$322B | -$462B | 43% | Treasury (2021-05) | Green Book FY2022 | `d11bf2c`, scored `6c9bfa2` |
| AGI surtax 1pp (>$20K single) | -$1,440B | -$797B | 45% | CBO Options 2025-2034 #46 (2024-12) | CBO Feb 2024 (matched) | `752f0f1`, scored `36d683f` |
| Corporate rate +1pp (21% to 22%) | -$136B | -$220B | 62% | CBO Options 2025-2034 #64 (2024-12) | CBO Feb 2024 (matched) | `752f0f1`, scored `36d683f` |
...
**CI gate.** `.github/workflows/validation-dashboard.yml` runs `python scripts/cold_holdout.py --max-mean-error 20 --min-within-25pct 21` as a blocking step. The workflow's own rule sets both: ceiling = ceil(mean x 1.25) to the nearest 5; floor = current count within 25%, minus one. On the post-#121 battery (26 cases, mean 15.2%, 22 within 25%) the rule gives a ceiling of **20** (15.2 × 1.25 = 19.0, rounded up to the nearest 5 is 20) and a floor of **21** — **the same pair Wave 5's 15.9% gave**, so PRs #119-#122 changed nothing here and the coordinator has nothing to move. The workflow was tightened to **20 / 21** after Wave 5 (PR #117), from the **25 / 20** Wave 4 had derived (PR #110), the **40 / 18** Wave 3 had derived, the **40 / 17** Wave 2 had derived, the **45 / 15** Wave 1 had derived and the **55 / 13** before that; a modelling lane never touches the yardstick, so the coordinator re-derives it in a separate PR once the docs are synced. Strict readiness (`scripts/check_readiness.py --strict`) no longer exempts Generic entries: an `Error` rating fails, and a `Poor` rating fails unless it carries a documented `known_limitations` note.
...
effect is right *there*. It is unsourced in magnitude on the other five
expenditure benchmarks, it is the single entry in `CONVENTION_EXCEPTIONS`, and
its size is measured: **+5.0% of the static effect on a SALT elimination and +20%
on Option 56**. Choosing it module-wide moves every fitted expenditure row and the
whole leave-one-out column together, so it is an owner decision.
...
| Policy | Official Score | Model Score | Error | Rating | Source |
|--------|----------------|-------------|-------|--------|--------|
| **Biden 21% to 28%** (fitted) | **-$1,347B** | **-$1,397B** | **3.7%** | **Excellent** | Treasury FY2025 Green Book, `line_item_differs` (**scope**) |
| Biden 21% to 28%, FY2022 window (**not fitted**) | -$858B | -$1,397B | 62.9% | Poor | Treasury FY2022 Green Book p. 104 |
| Trump 21% to 15% (**not fitted**) | [+$595B, +$673B] | +$1,492B | 121.6% | Poor | PWBM Table 1 / Tax Foundation Table 2 (via CRFB) |
...
cannot: the module's answer to "21% → 28%" is the same -$1,397.2B whichever decade is asked
about. It is **never to be fitted** — a second constant fitted here would make the pair
...
**Third, the Trump row's 121.6% is a distance from a document where 22.3% was a distance from
this model's own output, and about a third of it is scope.**
