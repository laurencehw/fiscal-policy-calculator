...
>
> Last Updated: September 13, 2026 (docs-sync: public headlines match the live 44-row Tier 1 battery after lane R3 / PR #169)

---

## Executive Summary

The model is benchmarked against **95 published estimates** — from CBO, JCT, Treasury and SSA, plus TPC, PWBM, the Tax Foundation and CRFB where no agency scored the policy — plus 4 *illustrations* with no official score at all, which are labelled and reported separately and never counted (`published_entries` vs `total_entries` on the scorecard; 99 total). Crucially, those benchmarks fall into **two epistemically different tiers**, and reporting them together overstates predictive power. Both are reproducible live: `python scripts/cold_holdout.py`. Tier 1 is additionally **pre-registered** (`fiscal_model/validation/preregistered.py`) and **CI-gated**.
...
> **44 out-of-sample cases, mean abs error 15.2%, 26/44 within 15%, 36/44 within 25%** (median 12.3%; error mass 668.6). PR #173 gave the three pre-2024 corporate rows the CBO Outlook receipts path their own edition was priced on (April 2018, September 2020, May 2022) — 99.7% → 53.4%, 93.1% → 21.8%, 49.2% → 41.6%, the 2024 row byte-identical — taking the corporate class 71.6% → 40.3% and the tier 18.0% → 15.2% on the same rows; what remains on all four corporate rows is one quantity, the module's 80.8% implied marginal base against JCT's 52.6–57.1%. Before #173 the tier read **18.0%, 26/44 within 15%, 35/44 within 25%** (mass 793.8). Lane R3 (PR #169) added 22 rows from CBO's 2018, 2020 and 2022 *Options* volumes, each scored on its own volume's decade, with all 22 prior rows byte-identical — the mean rose because the battery grew, not because any score moved; per class it read (before #173) corporate 71.6% (n=4), payroll 18.6% (8), ordinary rate 15.3% (11), capital gains 14.4% (7), AGI surtax 7.2% (4), enacted-law 7.4% (3), tax expenditure 7.1% (2), discretionary 4.6% (5). The 22-row reading before R3 was: **mean abs error 11.8%, 17/22 within 15%, 19/22 within 25%** (median 8.9%; error mass 258.9). The tier is **eight policy classes** and must never be quoted as one number — **live, after R3**: corporate 71.6% (n=4), payroll 18.6% (8), ordinary rate 15.3% (11), capital gains 14.4% (7), AGI surtax 7.2% (4), enacted-law 7.4% (3), tax expenditure 7.1% (2), discretionary 4.6% (5). The pre-R3 class list (corporate 44.5% n=1, AGI 5.2% n=2, ordinary 13.8% n=4) is not current. **Wave F moved one row and one class and nothing else**: PR #165 gave `cbo_opt45_top4_brackets_2pp` CBO's own year-indexed statutory bracket schedule and it went 14.3% → **17.9%**, a regression registered before the file was opened, which is the whole of 11.6% → 11.8% and of 12.9% → 13.8%.
> **All 44 rows are `line_item`.** The tier's `secondhand` count is **0** and its `model_estimate` count is **0**, which is new in Wave E and still holds after R3.
...
> **Read this section with lane R2's withdrawals in hand.** Four of the rows the group masses below are computed over are no longer in the battery, because their targets are in no publication, and a fifth moved onto CBO publication 58164's own option. The causes are unchanged and the *arithmetic* is stale until the next wave recomputes it; the live figures are `python scripts/cold_holdout.py` (44 cases, mean 18.0%, median 12.3%, error mass 793.8) and the per-class block it prints. What R2 removed from this accounting is **109.1 units of mass that was never measuring the model** — it was measuring the distance to four numbers this repository had written down itself.
...
> **This table is a historical snapshot and must not be quoted as the live battery.** Four of its nine rows were retired in Wave E for want of a published target (Medicare surcharge, 5pp top rate, 2pp cut, Warren surtax, plus `top_rate_45`); several remaining figures are superseded (1pp-all is −$1,081B not −$960B; Biden $400K is −$246B / 22% not −$252B / 13%). Live Tier 1 is **44 cases @ 18.0% mean / 12.3% median** in the executive summary above. `python scripts/cold_holdout.py` is the source of truth.
...
The three `Poor` rows are documented misses, not omissions — see the Tier 1 tail discussion above. **Do not read the retired Excellent/Good rows as live accuracy.**
