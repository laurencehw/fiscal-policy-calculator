...
**Tier 1 — out-of-sample predictions** (uncalibrated, bottom-up from SOI; the genuine test). **9 pre-registered cases, mean abs error 44.8%, 5/9 within 15%, 6/9 within 25%.** Never state this as a single "validated within X%" number — the distribution has a tight core (rate changes, 2–19%) and a long tail (one secondhand top-rate target at 118%, two capital-gains cases at 79%/154%). Ordinary-bracket rate changes (JCT 1pp, Biden $400K, top-rate 45%) score on the ordinary-income base; AGI-inclusive surtaxes (TPC $1M+/$500K+, Warren AGI>$2M, Medicare surcharge) score on the full taxable-income base — classified from how each source describes its base, never fitted (`cold_holdout.py --ordinary-base` shows the correction *worsens* the AGI-inclusive cases, the tell). The Biden $400K+ Generic path scores **−$284B vs −$252B official (~13%)** — earlier docs that cited "~−$250B / ~1%" were showing a hand-tuned figure, not the prediction. Capital-gains cases use ONE frozen elasticity set (module defaults 0.8/0.4), never the per-case tuples in `scenarios.py`; they miss badly and are kept as documented misses. Phase A's widening replaced an earlier 4-case ~8% figure measured on friendly shapes. Every case has a row in `fiscal_model/validation/preregistered.py`; the tier is CI-gated (`cold_holdout.py --max-mean-error 60 --min-within-25pct 5`) and Generic entries are no longer exempt from strict readiness. See [[fpc-review-roadmap]] and `planning/VALIDATION_EXPANSION.md`.
