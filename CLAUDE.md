...
**Tier 1 — out-of-sample predictions** (uncalibrated, bottom-up from SOI filer counts, module revenue identities and source-stated spending levels; the genuine test). **22 pre-registered cases, mean abs error 42.9% (median 22.1%), 6/22 within 15%, 12/22 within 25%.** Never state this as a single "validated within X%" number — the distribution has a tight core and a long tail, and each tail case has a documented structural cause: rate changes at conventional thresholds land at 1–21%, fast-spending discretionary funding cuts at 10–23%, and everything behavioural (capital gains, gains at death, payroll incidence, corporate margins) at 47–154%.

**Fourteen of the 22 are the CBO *Options for Reducing the Deficit: 2025–2034* battery** (publication 60557, Dec 2024) — **14 runnable alternatives across 11 of its 76 options**; the other **65 options** carry a one-line exclusion reason in `fiscal_model/validation/cbo_options.py`. Three exclusions are **leakage**, not missing machinery: Options 53, 56 and 62 would be scored by module constants calibrated to reproduce those same reforms. Spending options qualify only if CBO's *own* budget-authority path is a level `SpendingPolicy` can express (`is_level_budget_authority_path`, 25% tolerance). The battery is scored on `BaselineVintage.CBO_FEB_2024` via `validation/core.build_scorer_for_vintage()` — which changes none of the 14 scores, because every uncalibrated shape is bottom-up and none reads a level off the baseline.
...
**Tier 2 — calibrated reference models** (parameters tuned to reproduce the official decomposition; low error expected by construction). The calibrated tier is **46 benchmarks** and splits in two: **34 fitted** references at **2.7% mean** (33/34 within 15%), and **12 unfitted module reconstructions** — the Phase E international/trade/pharma/enforcement/climate presets, scored against published figures no module constant was ever fit to — at **394.1% mean / 57.1% median**. Never quote one for the other. By provenance the 46 are **9 `line_item` / 15 `line_item_differs` / 15 `secondhand` / 7 `model_estimate` / 0 `unclassified`** after the transcription pass. Read that before quoting any error as accuracy: **15 targets are now known to disagree with the document they cite** — the universal insulin cap in *sign* (CBO scores it +$11.4B to the deficit, not −$15B), TCJA AMT relief by 67% (the carried $450B looks like a five-year figure), FDII repeal against a Treasury row that nets to $0. No target was moved; the published figure rides on `ScorecardEntry.official_10yr_billions_line_item` and every one is listed in `docs/VALIDATION.md`. Another 15 could not be traced to any document, including both Social Security payroll targets (OCACT publishes only percent-of-payroll, no dollars, for E2.1 and E2.5). Held out, the fitted modules score **~59% mean / ~36% median over 18 leave-one-out cases** (4 more declared not cross-validatable, never folded in) — run `python scripts/run_loo.py`; report that separately from the by-construction figure, never as a replacement for it. Examples:
