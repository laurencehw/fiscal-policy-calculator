...
All core features, all four horizon features, the distributional-validation cycle, and the Ask assistant feature are complete (May 2026). **3322 tests passing across the model + Ask stack** (1 skipped; `python -m pytest tests/ -q`).
...
**Tier 1 — out-of-sample predictions** (uncalibrated, bottom-up from SOI filer counts, module revenue identities and source-stated spending levels; the genuine test). **26 pre-registered cases, mean abs error 18.0% (median 12.6%), 14/26 within 15%, 21/26 within 25%.** Never state this as a single "validated within X%" number — the distribution has a tight core and a long tail, and each tail case has a documented structural cause: rate changes at conventional thresholds land at 2–22%; discretionary funding changes, now spent out on a fitted account-class profile, land at 0–11% for the five CBO Options rows and 10–18% for the three enacted-law components; the tier's one **tax-expenditure cap**, CBO Option 56, lands at **13.1%** since Wave 4 (PR #105) gave the excess share CBO's own chained-CPI indexation instead of evaluating it once at `start_year` — the remaining residual is half a **base omission** (CBO caps premiums *and* FSA/HRA/HSA contributions; the repository's premium distribution has no account dimension) and about a fifth an **unsourced behavioural offset whose sign convention is the reverse of `TaxPolicy`'s**, both named rather than tuned; two rate cases whose source states a filing-status-specific boundary the generic path cannot express land at 18% and 45%. **The whole gains-at-death group moved in Wave 4** (PR #108), which gave the death channel the six carve-outs a realization-at-death proposal does not tax and a rate response at death: `treasury_capgains_39_plus_stepup_elim` 217.5% → **0.2%** — quote that only with the lane's own caveat that it is **two errors cancelling**, a death channel that came in nearly twice the pre-registered hand path's size closing a gap the hand path had left open the other way — `biden_capital_gains_39` 134.9% → **16.7%**, and `cbo_opt51_gains_at_death` **8.4% → 19.3%, worse, registered in advance as a regression** because its 8.4% was bought by taxing charitable bequests and small decedents' housing gains that no such regime reaches. What is left of the behavioural tail runs 45% (a 2pp preferential-rate change), 47% (corporate margins) and 54–56% (payroll incidence). **Capital gains is no longer the tier's dominant error mass**: 4 cases carrying 80.9 of 468.1 units (17.3%), down from 405.6 of 805.8 (50.3%); the two payroll rows are now the largest single mass at 109.6 (23.4%). The two Green Book rows land on *opposite* sides of their targets, which fired a pre-registered falsification test, and the diagnosis is not the one the test was written to catch: the ordering of the per-donor exclusion is pinned by a test and applying it first would move both the same way, so the cause is the **five-class decedent ladder** having no within-group dispersion — moving the exclusion $1M → $5M costs the model $82.2B of death channel where it costs Treasury $33.4B.
