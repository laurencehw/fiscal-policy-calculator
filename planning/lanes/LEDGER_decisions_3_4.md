...
1.6%** (median 0.0, 15/15 within 15%), **fitted held in place 26 @ 12.4%**
(median 2.4, 21/26), **reconstructions 38 @ 37.5%** (median 30.1, 11/38),
**retired 2 @ 397.2%**, **retired held in place 40 @ 55.5%** (median 33.6,
11/40). Note that the *fitted* held-in-place reading falls 27 → 26 rather than
rising: it folds back only rows a **target revision** moved out, and
`cap_charitable` left the fitted tier by **reclassification**, which is a
different mechanism and is deliberately not folded back. **Tier 1 is identical on both trees** — 26 @ 13.8%, median 10.6,
