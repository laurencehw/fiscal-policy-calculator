...
`inputs/elasticities/boehm_elasticities.csv` is a **time-varying import-demand elasticity**,
`final_path` rising 0.5517 (2025) → 0.8125 → 0.8967 → 0.9825 → 1.1483 → 1.2750 → 1.4750 → 1.7750 →
1.9175 → 2.0408 (2034–35), built from `elas_partial_yr` and `elas_full_yr` columns with a
`months_till_june` weight of 7. `inputs/elasticities/centered5_f2f_constructed_elas.csv` is 110
NAICS-4 foreign-to-foreign substitution elasticities (1111 → 4.642, 1112 → 3.626, …).
...
and **`dmyrevnfc = 0.85`** (nonfinancial corporates) — the share of the statutory rate that is
...
| **11** | **Source the tariff elasticity as a time path and the exporter absorption as a number.** | `fiscal_model/trade.py:148` (`import_price_elasticity = -0.997`, single scalar), `:142` (`border_pass_through_rate = 1.00`), `:172-174` (`high_tariff_threshold 0.30`, multiplier 2.0, floor 0.20 — **no CSV row and no citation**) | `conventional-tariff-analysis-model/inputs/elasticities/boehm_elasticities.csv` (0.5517 → 2.0408 over 2025–2035); `centered5_f2f_constructed_elas.csv` (110 NAICS-4 substitution elasticities); `config/default.yaml` `exporter_absorption: 5` | 2 | A short-run/long-run ramp where the app has one number. `Trade` is 43.6% (n=5) after PR #150 and **registered as worse on purpose**; four of the five rows improve to 35.66% on their own documents. Direction here is genuinely unknown in advance. | **None** — but note the app's −0.997 is Tax Foundation's own choice (FF861 p. 4), so moving to CBO's path swaps one published estimator for another rather than moving toward truth. State that. |
